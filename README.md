...
这是一个利用 **SakuraLLM (Qwen2.5-7B)** 大模型实现的本地字幕翻译方案。项目针对 NVIDIA RTX 30 系列显卡（以 3060 Ti 为例）进行了深度加速优化，翻译单集动漫（约 400 行）仅需 **70-100 秒**。
...
- **BATCH_SIZE**: 推荐设置为 8，这是 3060 Ti 兼顾显存占用与语境理解能力的“甜点位”。
